...
class IncidentEvent(BaseModel):
    ...
    action: str  # created, investigated, reported, acknowledged, assigned, resolve_submitted, resolve_verified, resolve_rejected, resolved, commented
    actor: str = "system"
    detail: str = ""
    timestamp: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
...
class ResolutionVerification(BaseModel):
    """Result of re-running the DQ check after a resolution attempt."""

    verified: bool
    confidence: str = ""  # HIGH, MEDIUM, LOW — how well the note matches the fix
    confidence_reason: str = ""
    still_failing_tests: List[str] = Field(default_factory=list)
    latest_error: str = ""
    checked_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))


class IncidentEvent(BaseModel):
    """A single lifecycle event in an incident's audit trail."""

    timestamp: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    action: str  # "created", "investigating", "reported", "acknowledged", "assigned", "resolve_attempted", "resolve_rejected", "resolved", "verified"
    actor: str = "system"
    detail: str = ""
...
class Incident(BaseModel):
    ...
    resolution_category: Optional[str] = None  # Schema/Data/Pipeline/Config/External
    acknowledged_at: Optional[datetime] = None
    resolved_at: Optional[datetime] = None
    verification_result: Optional[dict] = None  # VerificationResult as dict
    ...
    def add_event(self, action: str, actor: str = "system", detail: str = "") -> None:
        """Append a lifecycle event to the audit trail."""
        self.events.append(IncidentEvent(action=action, actor=actor, detail=detail))
        self.updated_at = datetime.now(timezone.utc)
    ...
    def transition(self, status: IncidentStatus) -> None:
        ...
        self.status = status
        self.updated_at = datetime.now(timezone.utc)
        self.events.append(IncidentEvent(action=status.value))
